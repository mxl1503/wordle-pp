...
const WordList& GetAnswerList() {
    static const WordList answerList = LoadWordList("wordLists/answerlist.txt");
    return answerList;
}

const WordList& GetGuessList() {
    static const WordList guessList = LoadWordList("wordLists/guesslist.txt");
    return guessList;
}
...
bool IsMatch(std::string_view word, std::string_view guess, std::string_view feedback) {
    ...
    for (std::size_t i = 0; i < guess.size(); ++i) {
        if (feedback[i] == 'Y') {
            ...
            charCount[guess[i]]--;
        } else if (feedback[i] == 'B' && charCount[guess[i]] > 0 &&
                   word.find(guess[i]) != std::string::npos) {
            return false;
        }
    }
...
WordList FilterAnswerList(
    const WordList& answerList,
...
std::string FindWorstFeedback(
    std::string_view guess,
    const WordList& refinedAnswerList
) {
